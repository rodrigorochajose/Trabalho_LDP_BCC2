...
<table style="border-collapse: unset; margin-left: 100px;">
	<tr>
		<td>
            <a href="receita_cadastrar.asp" class="waves-effect green accent-4 btn-small">
                <i class="material-icons left">add</i>
                Cadastrar <%= Titulo %> 
            </a>
			<a href="receita_listar.asp?Ativo=<% If Ativo = 1 Then %>0<% Else %>1<% End If %>" class="waves-effect <% If Ativo = 1 Then %>red darken-4<% Else %>green accent-4<% End If %> btn-small">
				<% If Ativo = 1 Then %> 
...
<table id="tabela_conteudo">
    <% While Not Reg.EOF %>
        <tr>
            ...
            <td style="width: 7%">
                <a href="receita_atualizar_script.asp?id=<%= Reg("id").Value %>&ativo=1" title="Informar Pagamento" class="btn-floating btn-large waves-effect green accent-4 botoes <% If Reg("ativo") = 1 Then %>disabled<% End If %>">
